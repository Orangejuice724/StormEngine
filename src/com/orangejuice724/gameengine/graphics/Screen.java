...
public class Screen {
	...
	public void renderGUI(int xPos, int yPos, int tile, int colour,
			int mirrorDir, int scale, int sheetId) {
		xPos -= xOffset;
		yPos -= yOffset;
		...
		for (int y = 0; y < sheets[sheetId].height; y++) {
			...
			for (int x = 0; x < sheets[sheetId].width; x++) {
				...
				if (col < 255) {
					for (int yScale = 0; yScale < scale; yScale++) {
						if (yPixel + yScale < 0
								|| yPixel + yScale >= sheets[sheetId].height)
							continue;
						for (int xScale = 0; xScale < scale; xScale++) {
							if (xPixel + xScale < 0
									|| xPixel + xScale >= sheets[sheetId].width)
								continue;
							sheets[sheetId].pixels[(xPixel + xScale)
									+ (yPixel + yScale) * sheets[sheetId].width] = col;
						}
					}
				}
			}
	...
	public void renderColour(int xPos, int yPos, int tile, int colour,
...
//						// = col;
//						pixels[xPixel + yPixel * width] = sheet.pixels[xSheet
//								+ ySheet * sheet.width];
//					}
//				}
//				// }
...
//		}
		...
		for (int yy = 0; yy < 8; yy++) {
			...
			for (int xx = 0; xx < 8; xx++) {
				...
				pixels[xp + yp * width] = sheet.pixels[xx + yy * 8];
			}
		}
	}
}
